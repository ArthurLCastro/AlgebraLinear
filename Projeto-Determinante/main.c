// Determinar Determinante de uma Matriz quadrada de qualquer ordem
// Arthur Castro
// Setembro de 2019
...
void determinante(){
    ...
    printf("\t\xAF O determinante da matriz\n");
    impressao();
    printf("\n\t e igual a: \n");
    printf("\n det(M) = %.2f \n", determ);
...
float detLaplace(){
    float detM = 0;
    unsigned int linha1 = 0, coluna;
    
    for(coluna=1; coluna<=ordemMat; coluna++){
        coluna = coluna -1;

        detM += (matriz[linha1][coluna]) * ((-1)^(1 + coluna )) * (cofator[linha1][coluna]);
    }
